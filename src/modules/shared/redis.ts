...
// In production, Redis is REQUIRED — tokens will never expire without it.
// In development/test, MockRedisClient is used (tokens don't expire — for dev only).
if (process.env.NODE_ENV === 'production' && !process.env.REDIS_URL) {
  console.error('[FATAL] REDIS_URL must be set in production. Exiting.');
  process.exit(1);
}
