...
export async function handleStreamableHTTP(req: Request, res: Response) {
  ...
  try {
    ...
    // incorrect or dead session for the authed user, return 404 so client creates a new one
    if (sessionId) {
      if (!(await isSessionOwnedBy(sessionId, userId))) {
        logger.warning('Session ownership mismatch - returning 404 to trigger new session', {
          sessionId,
          userId,
